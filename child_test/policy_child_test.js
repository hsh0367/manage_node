...
function table_view(dictdata) {
  ...
  if (chosechema == 'USER') {
    ...
    // var simcheck = 'imsi != ""';
    var now = Date.now();
    var sim_check = 'imsi.@size >=15 AND msisdn.@size >=8';
    ...
    var match_sim_check = 'imsi.@size >=15 AND msisdn.@size >=8 AND expire_match_date < ' + now + ' AND sim_expire_date > ' + now  + '';
    console.log(match_sim_check);
    let sim_checker = realm.objects('SIM').filtered(match_sim_check);
...
function user_info(dicdata) {
  ...
  if (user_checker.length > 0) {
    ...
    if (user_sim_checker.length > 0) {
      ...
      // var msg = command + "|" + sub_command + "|" + seq + "|" + user_sim_checker[0].msisdn + "|" + user_checker[0].user_sim_imsi + "|" + user_checker[0].credit + "|" + user_sim_checker[0].expire_match_date + "|" +
      //   user_sim_checker[0].sms_promo[0].net + ":" + user_sim_checker[0].sms_promo[0].other + ":" + user_sim_checker[0].sms_promo[0].expire_date + "|" +
      //   user_sim_checker[0].sms_promo[1].net + ":" + user_sim_checker[0].sms_promo[1].other + ":" + user_sim_checker[0].sms_promo[1].expire_date + "|" +
      //   user_sim_checker[0].sms_promo[2].net + ":" + user_sim_checker[0].sms_promo[2].other + ":" + user_sim_checker[0].sms_promo[2].expire_date + "|" +
      //   user_sim_checker[0].call_promo[0].net + ":" + user_sim_checker[0].call_promo[0].other + ":" + user_sim_checker[0].call_promo[0].expire_date + "|" +
      //   user_sim_checker[0].call_promo[1].net + ":" + user_sim_checker[0].call_promo[1].other + ":" + user_sim_checker[0].call_promo[1].expire_date + "|" +
      //   user_sim_checker[0].out_call_time + "|0|0|";
...
function fcm_key_renewal(dicdata) {
  ...
  if (user_serial_checker.length > 0) {
    ...
    // return command + "|" + sub_command + "|" + seq + "|" + tcp_fd + "|SUCCESS|";
...
function auto_flag_renewal(dicdata) {
  ...
  try {
    ...
    if (user_checker.length == 1) { //user 정보가 맞을경우
      ...
      console.log("auto_flag renewal success");
      ...
      if (user_credit >= sim_price) { // if credit is to buy
        ...
        if (user_sim_checker.length > 0) { // 심이 이미 있는경우
          ...
          if (user_sim_checker[0].expire_match_date > now) // 심을 연장할려는 경우
          {
            ...
            console.log("######## ", timeConverter(user_sim_checker[0].expire_match_date))
          ...
          if (match_sim_checker.length > 0) {
            ...
            process.send(msg);
            var db_user_msg = "DB|D05|" + user_checker[0].credit + "|" + user_checker[0].user_pid + "|" + timeConverter(match_sim_checker[0].expire_match_date) + "|" + match_sim_checker[0].imsi + "|" + user_checker[0].user_id + "|" +
