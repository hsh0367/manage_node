...
var user_key_map = hash_test.user_key_map;
...
function user_info(dicdata) {
  ...
  var user_check = user_map.get(user_serial)

  if (user_check != "undefined" && user_check.user_id == user_id) {
    var imsi = user_check.user_sim_imsi
    var sim_check = sim_map.get(imsi)

    var credit = user_check.credit

    if (sim_check != "undefined" && sim_check.user_id == user_id) {
      var msisdn = sim_check.msisdn
      var expire_match_date = sim_check.expire_match_date
      var out_call_time = sim_check.out_call_time
...
function register(dictdata) {
  ...
  var user_key = hash_test.get_user_key(user_id)
  var user_check = user_map.get(user_key)
  ...
  if (user_pw == "FACEBOOK" || user_pw.length != 0) { //회원가입이 페이스북으로 진행할경우 비밀번호에 FASCEBOOK 입력
    ...
    if (user_map.size == 0) {
      ...
      var user_pid = 400000 + user_map.size;
    }

    var user_serial = "PH01" + pad(user_pid, 15);

    if (user_check == "undefined") { // is not find user_id
      realm.write(() => {
...
function tt_login(login_data) {
  ...
  var user_key = hash_test.get_user_key(login_data.user_id)
  var user_check = user_map.get(user_key)

  write_log("user_checker.length : " + user_checker.length)
  if (user_check != "undefined" && user_check.user_pw == login_data.encpwd) { //if user is found
    var sim_check = sim_map.get(login_data.imsi)
    ...
    if (sim_check != "undefined" && sim_check.user_id == login_data.user_id) { //심구매 유저일경우

      sim_data.imsi = sim_check.imsi
      sim_data.msisdn = sim_check.msisdn
      console.log("policy_child login sim  o");

      bull_add_data("REALM|MODIFY|USER|" + user_check.serial + "|" + "user_serial = '" + user_check.serial + "'|login_time|INT|" + now + "|")
      ...
      if (user_check.user_sim_imsi != login_data.imsi && user_check.user_sim_imsi != '0') {

        bull_add_data("REALM|PDEATH|SIM|" + user_check.user_sim_imsi + "|")
        sim_map.delete(user_check.user_sim_imsi)
        //그전에 매칭되었던 심인 경우 삭제
      }


      var match_sim_check = sim_map.get(user_check.user_sim_imsi)

      //매치된 심이 있는지 확인
      if (match_sim_check != "undefined" && match_sim_check.user_id == user_check.user_id) {
        write_log("match_sim_checker.length : " + match_sim_check.size)
        bull_add_data("REALM|PDEATH|SIM|" + user_check.user_sim_imsi + "|")
        bull_add_data("REALM|INSERT|SIM|" + user_check.user_sim_imsi + "|" + user_check.user_sim_imsi + "|" + login_data.imsi + "|" + imei + "|" + mcc + "|" + mnc + "|" + sim_data.join_type + "|")
        bull_add_data("REALM|MODIFY|USER|" + user_check.serial + "|" + "user_serial = '" + user_check.serial + "'|user_sim_imsi|STRING|" + login_data.imsi + "|")
      }
      else {
...
function login(dicdata) {
  ...
  var user_key = hash_test.get(login_data.user_id)
  var user_check = user_map.get(user_key)

  // var facebook_check = 'user_id = "' + login_data.user_id + '"';

  if (user_check != "undefined" && user_check.user_pw == login_data.encpwd) { //if user is found
    if (user_check.join_type == 1) { // ttgo 일경우
      tt_login(login_data)
    }
    else {
...
function fcm_key_renewal(dicdata) {
  ...
  if (user_check != "undefined") {
    ...
    bull_add_data("REALM|MODIFY|USER|" + user_check.serial + "|" + "user_serial = '" + user_check.serial + "'|fcm_push_key|STRING|" + fcm_key + "|")
...
function voip_key_renewal(dicdata) {
  ...
  if (user_check != "undefined") {
    bull_add_data("REALM|MODIFY|USER|" + user_check.serial + "|" + "user_serial = '" + user_check.serial + "'|fcm_push_key|STRING|" + voip_key + "|")
...
function auto_flag_renewal(dicdata) {
  ...
  try {
    ...
    if (user_check != "undefined" && user_check.user_id == id) { //user 정보가 맞을경우

      var user_pid = user_check.user_pid
      bull_add_data("REALM|MODIFY|USER|" + user_check.serial + "|" + "user_serial = '" + user_check.serial + "'|auto_flag|INT|" + auto_flag + "|")
...
//           console.log("sim_check.expire_match_date : " + sim_check.expire_match_date)
//
//
//           sim_data.msisdn = sim_check.msisdn;
//           sim_data.expire_match_date = sim_check.expire_match_date + sim_data.add_time;
//           sim_data.imsi = sim_check.imsi;
//
...
//             sim_check.expire_match_date = sim_data.expire_match_date;
//           });
//
//
...
//             realm.write(() => {
//               user_check.credit = sim_data.credit; //크래딧 차감
//               user_check.user_sim_imsi = sim_data.imsi;
//               match_sim_checker[0].user_pid = user_check.user_pid;
//               match_sim_checker[0].user_id = user_check.user_id;
//               match_sim_checker[0].user_serial = user_check.user_serial;
...
function top_up(dictdata) {
  ...
  //UPDATE tb_user_test set credit = credit + 7500, charge_cnt = charge_cnt + 1, coupon =coupon+1 WHERE pid = user_check.user_pid


  var user_check = 'user_id = "' + user_id + '" AND user_serial = "' + user_serial + '"';
  var user_checker = realm.objects('USER').filtered(user_check);

  var user_sim_check = 'user_pid =  "' + user_check.user_pid + '"';
  ...
  if (user_check != "undefined" && user_check.user_id == user_id) {
    var imsi = user_check.user_sim_imsi;
    var sim_check = sim_map.get(imsi)
    if (sim_check != "undefined" && sim_check.user_pid == user_check.user_pid) {
      var credit = user_check.credit;
      var user_pid = user_check.user_pid;
      var user_id = user_check.user_id;
      var user_serial = user_check.user_serial;
      var credit = user_check.credit;
      var credit = user_check.credit;
      var msisdn = sim_check.msisdn;
      credit = credit + topup_price;
      realm.write(() => {
        user_check.credit = credit
      })
